...
/**
 * 
 * Classe bouton poussoir qui creer des bontons en style "flat".
 * Ses bontons ont 2 couleurs afin de changer de couleur lors du clique.
 * Ce sont des boutons de type : bouton poussoir
 * 
 * @author JM
 *
 */
public class BoutonPoussoir {
	...
	public BoutonPoussoir (boolean valeur, int x1_portrait, int y1_portrait,int x2_portrait, int y2_portrait, 
	...
	public void dessine_portrait(Canvas canvas) {
		int hauteur_texte = (y2_portrait_-y1_portrait_)/2;
		Paint paint = new Paint();
		if(!pre_click && !block) paint.setColor(couleur_1_);
 	   ...
 	   if(!pre_click) canvas.drawText(texte_1_,(x2_portrait_+x1_portrait_)/2-(int)paint.measureText(texte_1_)/2,(y2_portrait_+y1_portrait_)/2+hauteur_texte/2,paint);
	...
	public boolean estDansBouton_portrait(int x, int y) {
		if(x1_portrait_<x && x<x2_portrait_ && y1_portrait_<y && y<y2_portrait_ && !block) return true;
		return false;
	}
